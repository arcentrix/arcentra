...
// ProviderSet provides the pipeline engine and its Kafka task queue producer.
var ProviderSet = wire.NewSet(
	ProvideEngine,
	ProvideTaskQueueProducer,
)

// ProvideEngine creates the pipeline engine with all required dependencies.
func ProvideEngine(
	...
	return NewEngine(repos, pluginMgr, taskQueue, st, logger, appConf, services.Secret)
}
...
func ProvideTaskQueueProducer(appConf *config.AppConfig, logger *log.Logger) nova.TaskQueue {
	...
	if kafkaCfg.BootstrapServers == "" {
		log.Info("Kafka not configured, pipeline engine running in local-only mode")
		return nil
	}
