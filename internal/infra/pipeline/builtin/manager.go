...
type Manager struct {
	mu       sync.RWMutex
	handlers map[string]map[string]pipeline.ActionHandler // builtin:action -> handler
	infos    map[string]*pipeline.BuiltinInfo
	logger   log.Logger
}
...
func NewManager(logger log.Logger) *Manager {
	bm := &Manager{
		handlers: make(map[string]map[string]pipeline.ActionHandler),
		infos:    make(map[string]*pipeline.BuiltinInfo),
		logger:   logger,
	}
	bm.registerBuiltins()
	return bm
...
func (m *Manager) registerBuiltins() {
	...
	m.registerBuiltin("reports", &pipeline.BuiltinInfo{
		Name:        "reports",
	...
	m.registerBuiltin("stdout", &pipeline.BuiltinInfo{
		Name:        "stdout",
	...
	m.registerBuiltin("scm", &pipeline.BuiltinInfo{
		Name:        "scm",
...
func (m *Manager) registerBuiltin(name string, info *pipeline.BuiltinInfo, handlers map[string]pipeline.ActionHandler) {
	m.mu.Lock()
	defer m.mu.Unlock()

	if m.handlers[name] == nil {
		m.handlers[name] = make(map[string]pipeline.ActionHandler)
	}
...
func (m *Manager) Execute(ctx context.Context, builtin, action string, params json.RawMessage, opts *pipeline.BuiltinOptions) (json.RawMessage, error) {
	m.mu.RLock()
...
func (m *Manager) GetInfo(name string) (*pipeline.BuiltinInfo, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *Manager) ListBuiltins() map[string]*pipeline.BuiltinInfo {
	m.mu.RLock()
	defer m.mu.RUnlock()

	result := make(map[string]*pipeline.BuiltinInfo)
