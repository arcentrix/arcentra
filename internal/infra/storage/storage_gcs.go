...
func newGCS(s *Storage) (*GCSStorage, error) {
	...
	// AccessKey 为服务账号 JSON 凭据文件路径；按服务账号类型解析后交给 option.WithCredentials（避免弃用的 WithCredentialsFile/JSON）。
	if s.AccessKey != "" {
		credJSON, readErr := os.ReadFile(s.AccessKey)
		if readErr != nil {
			return nil, readErr
		}
		creds, credErr := google.CredentialsFromJSONWithType(
			context.Background(),
			credJSON,
