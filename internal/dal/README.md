...
**DAL**（Data Access Layer，数据访问层）：存放数据库 schema、手写 SQL 与 sqlc 生成代码，与 `internal/infra/persistence` 中的仓储实现配合使用。
